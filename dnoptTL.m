% TOMLAB DNOPT 4 NLP Solver
%
% function Result = dnoptTL(Prob)
%
% INPUT:
...
% moremem   Add more memory if dnopt stops with not enough storage message.
...
% nnObj     Variable i=1,...,nnObj have a non constant derivative df(x) / dx(i)
...
%
% dnopt keywords in optPar(#) - and the fields used in optParam
%
...
% 10. MAJOR OPTIMALITY TOLERANCE    >0       max(2E-6,(10*eps_R)^0.5)
%                                            eps_R == optPar(41)
...
%     Option 1 changed to QP Scaling 0 by dnopt if no linear constraints
% 19. SCALE TOLERANCE               >0       0.9       <1
% 20. SCALE PRINT                   0        0         1       1 = True
%     (The value of SCALE PRINT is not shown in the dnopt print file)
...
%     Is set by dnoptTL dependent on Prob.ConsDiff, Prob.NumDiff
...
%
% 72. SYSTEM INFORMATION            0       0          1  
%                                   Use in conjunction with 20 SCALE PRINT
%                                   
% -----------------------------------------------------------------------
%
% OUTPUT:
...
% ExitFlag Exit status from dnopt.m (similar to TOMLAB).
% Inform   DNOPT information parameter.
...
% Solver   Name of the solver (dnopt).
...
% Copyright (c) 2000-2014 by Tomlab Optimization Inc., $Release: 8.0.0$
% Written July 4, 2000.   Last modified Jun 27, 2014.
...
if nargin < 1, error('dnoptTL needs the Prob structure as input');end
...
Result.Solver='dnopt';
Result.SolverAlgorithm='DNOPT 4.1-1 NLP code';

PriLev=Prob.PriLevOpt;

% Define lower and upper bound arrays for DNOPT
%
...
if isfield(Prob.QP,'c')
   ...
   elseif strcmpi(Prob.FUNCS.f,'lp_f')
      ...
      m3 = length(c) > 0;
      nnObj = 0;
   elseif isfield(Prob.SOL,'nnObj')
      nnObj = Prob.SOL.nnObj;
   else
      c  = [];
      m3 = 0;
   end
...
if m2 > 0
   ...
   if isempty(Prob.ConsPattern)
      ...
      %nnJac       = size(gJac,2); 
      nnJac        = max(iy);         % Only nonlinear variables
      %gJac         = gJac(:,1:nnJac); % Squeeze gJac
   end
   % Must call function, in case global values are needed for constraints
   % Because dnopt calls constraints first, then functions
...
if isempty(gJac)
   ...
   if isempty(Prob.A)
      ...
      if size(gJac,2) < n
         A = sparse([[gJac,zeros(m2,n-size(gJac,2))];Prob.A]);
      else
...
callback = DefPar(Prob.SOL,'callback',[]);

dims = [n, m1, m2, nnObj, nnJac];
   
[hs, xs, pi, Inform, nS, nInf, sInf, Obj, iwCount, gObj, fCon, gCon] = ...
     dnopt( full(A), bl, bu, dims, Prob, iObj, optPar, ...
            Warm, hs, xs, pi, nS, ...
            Prob.SOL.SpecsFile, Prob.SOL.PrintFile, Prob.SOL.SummFile, ...
            PriLev, ObjAdd, moremem, Prob.Name, callback );
...
% v2=pi(m2+1:m2+m1);
% v3=pi(1:m2);
% if isempty(v2), v2=[]; end
% if isempty(v3), v3=[]; end
% Result.v_k=[v1;v2;v3];
Result.cJac = gCon;
...
Result.iwCount   = iwCount;

optParam = Prob.optParam;
if m1 > 0
   Result.Ax = Prob.A*xs(1:n);
...
% 060817 hkh  SNOPT is sloopy, violating simple bounds up to SOL.optPar(10)
% 060817 hkh  Violation could be avoided, now in comments
% 061025 med  No callbacks anymore if objective is linear
% 061224 med  moremem help updated
% 080213 hkh  Removed SOLSet call, Already done:set, comment optPar 39,40,48
...
% 090912 hkh  Return iwCount for use in multiMin, multiMINLP
% 091207 hkh  Set nnJac to only nonlinear variables, squeeze gJac, save huge CPU
%             time if many linear variables
% 100626 hkh  Avoid squeezing dimension of gJac, still expanded later
% 110914 hkh  Add check on field Prob.SOL.nnObj, user setting of nnObj
% 140627 ango Added parameter 72. System Information
